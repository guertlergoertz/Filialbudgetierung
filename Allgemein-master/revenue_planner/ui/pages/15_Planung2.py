"""Planung ausführen — LOGIK 2 (alternative Berechnung, Tabelle planung2)."""
...
from planning.engine import PlanParams, DayPlan
...
st.title("Planung ausführen — Logik 2")
st.caption(
    "Zweite, eigenständige Berechnungslogik (Monatsumsatz-basiert). "
    "Ergebnisse werden getrennt in `planung2` gespeichert, damit beide Logiken "
...
_eng = PlanningEngine2(conn, params)
...
run_mode = st.radio("Ausführen für", ["Alle Filialen", "Auswahl"], key="p2_runmode")
if run_mode == "Auswahl":
    selected_fils = st.multiselect("Filialen auswählen", all_filialen,
                                   placeholder="Filialen auswählen...", key="p2_sel")
else:
...
if st.button("🚀 Planung berechnen (Logik 2)", type="primary", disabled=not selected_fils,
             key="p2_run_btn"):
    if existing_n > 0:
...
if st.session_state.get("_confirm_replan2"):
    st.warning(
        f"Es existieren bereits **{existing_n:,}** geplante Tage (Logik 2) für **{planjahr}**. "
...
if st.session_state.get("_do_plan2"):
    ...
    try:
        engine = PlanningEngine2(conn, params)
        aktive_fils = [
        ...
        st.success(f"✅ {n_total} Filiale(n){skip_hint} — {len(results):,} Tage berechnet (Logik 2).")
...
if _use_fresh or _db_df is not None:
    ...
    st.divider()
    st.subheader("Ergebnisvorschau — Monatsübersicht (Logik 2)")
    if not _use_fresh:
        st.caption(f"Gespeicherte Planung (Logik 2) aus der Datenbank ({existing_n:,} Tage)")
    ...
    if _use_fresh and results is not None:
        ...
        st.download_button(
            label="📥 Excel-Planung herunterladen (Logik 2)",
            data=excel_bytes,
            file_name=f"Umsatzplanung_Logik2_{planjahr}_{gmbh.replace(' ', '_')}.xlsx",
...
if existing_plan and existing_plan["n"] > 0:
    st.caption(
        f"Gespeicherte Planung (Logik 2) für {planjahr}: {existing_plan['n']:,} Zeilen "
