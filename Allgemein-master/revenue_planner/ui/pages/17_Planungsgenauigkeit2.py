"""Planungsgenauigkeit — LOGIK 2: Plan vs. IST-Umsatz (Tabelle planung2)."""
...
st.title("Planungsgenauigkeit — Logik 2")
st.caption("Plan (zweite Berechnungslogik) vs. tatsächlicher IST-Umsatz.")

plan_rows = conn.execute(
    "SELECT * FROM planung2 WHERE CAST(strftime('%Y', datum) AS INTEGER)=? ORDER BY fil_nr, datum",
    (planjahr,),
).fetchall()
...
with pd.ExcelWriter(buf, engine="openpyxl") as writer:
    disp.to_excel(writer, index=False, sheet_name="Planungsgenauigkeit2")
    from openpyxl.styles import Font
    from openpyxl.utils import get_column_letter
    ws = writer.sheets["Planungsgenauigkeit2"]
    for cell in ws[1]:
...
st.download_button(
    ...
    data=buf.getvalue(),
    file_name=f"Planungsgenauigkeit_Logik2_{planjahr}{suffix}_{gmbh.replace(' ', '_')}.xlsx",
